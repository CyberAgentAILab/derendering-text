...
def get_geomap(scoremap, rectangles, instance_mask, scale=0.25):
    ...
    for i in range(len(rectangles)):
        ...
        angle = np.arctan((p2_rect[1] - p3_rect[1]) /
                          max(p2_rect[0] - p3_rect[0], 1))
...
class DefaultTextDataLoader(object):
    ...
    def set_basic_info(
        ...
        self.save_data_info = pickle.load(
            open(
                os.path.join(
                    self.data_dir,
                    'save_data_info.pkl'),
    ...
    def check_valid_id(self, index):
        ...
        file_names.append(
            os.path.join(
                self.image_dir,
                f'{self.prefixes[index]}.jpg'))
        file_names.append(
            os.path.join(
                self.metadata_dir,
                f'{self.prefixes[index]}.pkl'))
        flag = 0
    ...
    def get_text_rectangles_from_metadata(self, metadata):
        ...
        text_rectangles = text_rectangles.reshape(
            [8, numOfWords], order='F').T.astype(np.int32)
    ...
    def get_char_rectangles_from_metadata(self, metadata):
        ...
        char_rectangles = np.expand_dims(
            char_rectangles, axis=2) if (
            char_rectangles.ndim == 2) else char_rectangles
        char_rectangles = char_rectangles.reshape(
            [8, numOfchar], order='F').T.astype(np.int32)  # num_words * 8
    ...
    def get_char_cls_mask(self, cpoly, imgsize, transchar):
        ...
        for i in range(len(cpoly)):
            ...
            if char in self.label_dict_keys:
                cv2.fillPoly(char_cls_mask,
                             cpoly[i:i + 1],
                             int(self.label_dict[char]) + 1)
    ...
    def load_ocr_data(self, index, metadata, imgsize):
        ...
        text_rectangles, numOfWords = self.get_text_rectangles_from_metadata(
            metadata)
        char_rectangles, numOfchar = self.get_char_rectangles_from_metadata(
            metadata)
        transchar = self.get_transchar(transcripts)
        tpoly = text_rectangles.reshape(
            numOfWords, 4, 2)  # num_of_words * 4 * 2
    ...
    def load_alpha(self, img_size, index):
        ...
        if os.path.isfile(file_name):
            ...
            alpha[:, :, 1] = _alpha[:, :, 0]
            alpha[:, :, 1] = np.maximum(
                alpha[:, :, 1] - alpha[:, :, 0] - alpha[:, :, 2], np.zeros_like(alpha[:, :, 1]))
        else:
            alpha = np.zeros((img_size[1], img_size[0], self.effect_num))
        return alpha

    def get_font_data(self, metadata, text_num):
        font_label_vec = np.zeros(
            (self.pool_num,), dtype=np.float32) + self.ignore_label
    ...
    def get_stroke_data(self, metadata, text_num):
        stroke_param_num = 1
        stroke_param_vec = np.zeros(
            (self.pool_num, stroke_param_num), dtype=np.float32) + self.ignore_label
        stroke_visibility_vec = np.zeros(
            (self.pool_num,), dtype=np.float32) + self.ignore_label
        div_num = 5
        for i in range(text_num):
            if (i >= self.pool_num):
                continue
            if not metadata.effect_visibility[i].stroke_visibility_flag:
                stroke_visibility_vec[i] = 0
            else:
                stroke_visibility_vec[i] = 1
            stroke_size = 25 * (metadata.effect_params[i].stroke_param.border_weight / float(
                metadata.font_data[i].font_size) - 0.01)
            stroke_param_vec[i, 0, ] = min(
                (stroke_size * div_num * div_num) // div_num, div_num - 1)
        return stroke_param_vec, stroke_visibility_vec

    def get_shadow_data(self, metadata, text_num):
        shadow_param_num = 5
        shadow_param_vec = np.zeros(
            (self.pool_num, shadow_param_num), dtype=np.float32) + self.ignore_label
        shadow_visibility_vec = np.zeros(
            (self.pool_num,), dtype=np.float32) + self.ignore_label
        for i in range(text_num):
            if (i >= self.pool_num):
                continue
            if not metadata.effect_visibility[i].shadow_visibility_flag:
                shadow_visibility_vec[i] = 0
            else:
                shadow_visibility_vec[i] = 1
            shadow_param_vec[i, 0] = float(
                metadata.effect_params[i].shadow_param.opacity)
            shadow_param_vec[i, 1] = 2 * float(
                metadata.effect_params[i].shadow_param.blur) / metadata.font_data[i].font_size
            shadow_param_vec[i, 2] = 10 * float(
                metadata.effect_params[i].shadow_param.dilation) / metadata.font_data[i].font_size  # 0~0.25
            shadow_param_vec[i, 3] = (float(
                metadata.effect_params[i].shadow_param.offset_x) / metadata.font_data[i].font_size * 5)  # -0.5~0.5
            shadow_param_vec[i, 4] = (float(
                metadata.effect_params[i].shadow_param.offset_y) / metadata.font_data[i].font_size * 5)
...
class TextParserLoader(torch.utils.data.Dataset):
    def __init__(self, data_list, text_pool_num=10, char_pool_num=40):
        ...
        self.norm_charnetinp = util.Normalize(
            mean=(
                0.485, 0.456, 0.406), std=(
                0.229, 0.224, 0.225))
        self.norm_alphainp = util.Normalize(
            mean=(
                0.5, 0.5, 0.5), std=(
                0.5, 0.5, 0.5))
        self.dataset_list, self.dataset_priors, self.dataset_options = get_setup_data(
            data_list, char_dict, label_dict, text_pool_num)
    ...
    def get_ocr_data(self, text_char_data, rectangle_data):
        ...
        text_rectangles_array[0:min(len(text_rectangles), self.text_pool_num), :] = np.array(
            text_rectangles)[0:min(len(text_rectangles), self.text_pool_num), :]
        char_rectangles_array = np.zeros((self.char_pool_num, 8))
        char_rectangles_array[0:min(len(char_rectangles), self.char_pool_num), :] = np.array(
            char_rectangles)[0:min(len(char_rectangles), self.char_pool_num), :]
        ...
        # get geometory map
        text_geomap, text_scoremap_qt = get_geomap(
            text_scoremap, text_rectangles, text_ins_mask)
        char_geomap, char_scoremap_qt = get_geomap(
            char_scoremap, char_rectangles, char_ins_mask)
        # ocr data
        text_level_data = (
            text_fg_mask,
            text_ins_mask,
    ...
    def load_data(self, index, dataset):
        ...
        text_char_data = dataset.load_ocr_data(
            index, metadata, (img_org.shape[1], img_org.shape[0]))
    ...
    def __getitem__(self, index):
        ...
        # load data
        img_org, metadata, text_char_data, alpha = self.load_data(
            index, dataset)
        (text_rectangles, text_ins_mask, text_scoremap, char_rectangles,
         char_ins_mask, char_scoremap, char_cls_mask) = text_char_data
        ...
        output_array, output_rectangles = self.transform(
            (input_array, input_rectangles))
        [img_org, text_ins_mask, char_ins_mask, char_cls_mask,
            text_scoremap, char_scoremap, alpha] = output_array
        # pre-processing and reshape
        img_norm, img_alpha = self.pre_process(img_org)
        alpha = alpha.transpose(2, 0, 1)
        squeeze_outs = self.all_squeeze(
            (text_ins_mask,
             char_ins_mask,
             char_cls_mask,
             text_scoremap,
             char_scoremap))
        # get ocr data
        ocr_data, valid_text_index, text_num = self.get_ocr_data(
            squeeze_outs, output_rectangles)
        # get style data
        style_data = self.get_style_data(
            dataset, metadata, text_num, alpha, valid_text_index)
...
class InpaintorLoader(torch.utils.data.Dataset):
    def __init__(self, load_data_path, load_id):
        super().__init__()
        char_dict, label_dict = load_char_label_dicts()
        self.transform = transforms.Compose([
            util.RandomResizeWithBB(640, 640),
        ...
        self.dataset_list, self.dataset_priors = cfg.get_dataset_list(
            DefaultTextDataLoader, load_data_path, char_dict, label_dict)
    ...
    def get_coords_for_paste(self, alpha_aug, alpha):
        ...
        return (aug_y0, aug_y1, aug_x0,
                aug_x1), (tmp_y0, tmp_y1, tmp_x0, tmp_x1)

    def aug_alpha(self, img, alpha, dataset):
        aug_alpha_num = random.randint(1, 5)
        alpha_aug = np.zeros((alpha.shape[0], alpha.shape[1], aug_alpha_num))
        alpha_aug[:, :, 0] = np.max(alpha, 2)
        for i in range(1, aug_alpha_num):
            alpha = dataset.load_alpha(
                img, random.randint(
                    0, len(
                        dataset.prefixes) - 1))
            alpha = np.max(alpha, 2)
            (aug_y0, aug_y1, aug_x0, aug_x1), (tmp_y0, tmp_y1, tmp_x0,
                                               tmp_x1) = self.get_coords_for_paste(alpha_aug, alpha)
            alpha_aug[aug_y0:aug_y1, aug_x0:aug_x1,
                      i] = alpha[tmp_y0:tmp_y1, tmp_x0:tmp_x1]
