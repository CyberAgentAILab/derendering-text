...
class FullModel_dis(nn.Module):
    ...
    def forward(self, inputs):
        ...
        inpaint_out_crop = inpaint_out[:, :, crop_y:crop_y + 128, crop_x:crop_x + 128]
        d_fake_crop = self.dis(inpaint_out_crop)

        bgimg=F.interpolate(bgimg,(img_size_for_crop,img_size_for_crop),mode='bilinear')
        img_smooth=F.interpolate(img_smooth,(img_size_for_crop,img_size_for_crop),mode='bilinear')
        
        bgimg_crop = bgimg[:, :, crop_y:crop_y + 128, crop_x:crop_x + 128]
        img_smooth_crop = img_smooth[:, :, crop_y:crop_y + 128, crop_x:crop_x + 128]
...
class FullModel_gen(nn.Module):
    ...
    def forward(self, inputs):
        ...
        de2im_loss0 = F.l1_loss(de2im[0], F.interpolate(bgimg,de2im[0].shape[2:4],mode='bilinear'))
        de2im_loss1 = F.l1_loss(de2im[1], F.interpolate(img_smooth,de2im[1].shape[2:4],mode='bilinear'))
        de2im_loss = de2im_loss0 + de2im_loss1
        inp_loss = F.l1_loss(inpaint_out,F.interpolate(bgimg,inpaint_out.shape[2:4],mode='bilinear'))
        d_fake = self.dis(inpaint_out)
        inpaint_out_crop = inpaint_out[:, :, crop_y_256:crop_y_256 + 128, crop_x_256:crop_x_256 + 128]
        d_fake_crop = self.dis(inpaint_out_crop)
        g_loss = torch.mean(d_fake.mean(dim = 1))+torch.mean(d_fake_crop.mean(dim = 1))
...
ADAM=1
class Trainer():
    def __init__(self, inpaintor, discriminator, data_list, save_data_path, model_id, text_pool_num=10, batch_size=1, nworker=2):
        ...
        self.train_epoch_num = 400
        self.param_generatice = [param for name, param in inpaintor.named_parameters() if bool(re.fullmatch(self.layer_regex["generative"], name))]
        self.param_dis = [param for name, param in discriminator.named_parameters() if bool(re.fullmatch(self.layer_regex["dis"], name))]
        self.optimizer_gen = torch.optim.Adam(self.param_generatice, lr=0.0002, betas=(0.5, 0.999))
        self.optimizer_dis = torch.optim.Adam(self.param_dis, lr=0.0002, betas=(0.5, 0.999))
        ...
        self.schedulers = []
        self.schedulers.append(get_scheduler(self.optimizer_gen, self.train_epoch_num))
        self.schedulers.append(get_scheduler(self.optimizer_dis, self.train_epoch_num))
    ...
    def train_model(self):
        ...
        dataloader = torch.utils.data.DataLoader(
            self.dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.nworker, pin_memory=True)
        ...
        for epoch in range(0, epochs):
            ...
            if (epoch % 5 == 0):
                ...
                torch.save(self.inpaintor.state_dict(), os.path.join(self.savedir,'inp_{}.pth'.format(epoch)))
                torch.save(self.discriminator.state_dict(), os.path.join(self.savedir,'dis_{}.pth'.format(epoch)))
                torch.save(self.optimizer_gen.state_dict(), os.path.join(self.savedir,'optg_{}.pth'.format(epoch)))
                torch.save(self.optimizer_dis.state_dict(), os.path.join(self.savedir,'optd_{}.pth'.format(epoch)))
                torch.cuda.empty_cache()
    ...
    def train_step(self, inputs, end):
        ...
        inpaint_out, de2im, inp_loss, de2im_loss, loss_gen = self.fullmodel_gen(inputs)
        ...
        if (self.step % 100 == 0):
            ...
            log.debug(data_show)
            self.show_loss(torch.mean(inp_loss).item(), torch.mean(de2im_loss).item(), torch.mean(loss_gen).item(), 
                           torch.mean(loss_dis).item())
    def show_loss(self, inp_loss, de2im_loss, loss_gen, loss_dis):
        inp_loss_show="inp_loss: {:.3f} de2im_loss: {:.3f} loss_gen: {:.3f}".format(inp_loss, de2im_loss, loss_gen)
    ...
    def save_contents(self, inputs, text_inpaint_pred, de2im):
        ...
        img_save = (img_norm.data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5)*255
        cv2.imwrite(sn, img_save)
        sn = os.path.join(
            self.savedir, 'is_{}_{}.jpg'.format(self.epoch, self.cnt))
        img_save = (img_smooth.data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5)*255
        cv2.imwrite(sn, img_save)
        sn = os.path.join(
            self.savedir, 'bg_{}_{}.jpg'.format(self.epoch, self.cnt))
        bgimg_save = (bgimg.data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5)*255
        bgimg_saver = bgimg_save.copy()
        cv2.imwrite(sn, bgimg_save)
        
        sn = os.path.join(self.savedir, 'inp0_{}_{}.jpg'.format(self.epoch, self.cnt))
        inpimg_save = (text_inpaint_pred.data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5)*255
        inpimg_save = cv2.resize(inpimg_save,(256,256))
        cv2.imwrite(sn, inpimg_save)
        sn = os.path.join(self.savedir, 'alpha_{}_{}.jpg'.format(self.epoch, self.cnt))
        inpimg_save = (alpha.data.cpu().numpy()[0,0])*255
        inpimg_save = cv2.resize(inpimg_save,(256,256))
        cv2.imwrite(sn, inpimg_save)
        sn = os.path.join(self.savedir, 'inpd0_{}_{}.jpg'.format(self.epoch, self.cnt))
        inpimg_save = de2im[0].data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5        
        inpimg_save = (cv2.resize(inpimg_save,(256,256),interpolation=cv2.INTER_NEAREST))*255
        cv2.imwrite(sn, inpimg_save)
        sn = os.path.join(self.savedir, 'inpd1_{}_{}.jpg'.format(self.epoch, self.cnt))
        inpimg_save = de2im[1].data.cpu().numpy()[0].transpose(1,2,0)[:,:,::-1]*0.5+0.5        
        inpimg_save = (cv2.resize(inpimg_save,(256,256),interpolation=cv2.INTER_NEAREST))*255
        cv2.imwrite(sn, inpimg_save)
