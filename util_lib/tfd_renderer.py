...
def draw_text(canvas, tfd, bg_img, draw_offset_y, draw_offset_x, font, text, text_top, i, angle=0):
    ...
    fill_alpha = skp.get_fill_alpha(height,width, textblob, float(draw_offset_x), float(draw_offset_y),angle)
    stroke_alpha = skp.get_stroke_alpha(height,width, textblob, float(draw_offset_x), float(draw_offset_y), stroke_param, angle)
...
def render_tfd(tfd):
    ...
    for i, text in enumerate(tfd.texts):
        ...
        canvas = draw_text(canvas, tfd, bg_img, draw_offset_y, draw_offset_x, font, text, text_top, i)
        canvas.resetMatrix()
